...
- Provide a single Vite Task that hot-reloads the backend and website and rebuilds/reloads the extension during development.
...
- Production clients connect only to the secure WebSocket endpoint configured in `config/urls.ts` (`wss://www.jelly-party.com` by default). The protocol may be changed freely; there is no compatibility code for the old backend or extension.
- One Cloudflare Worker deployment serves the static website and `/join` route, exposes the health and WebSocket endpoints, and routes each party to one Durable Object. Static assets bypass Worker execution. The object validates incoming message shapes and sensible size limits, broadcasts party/chat/playback messages, persists the latest 10,000 messages as paged chat history in its embedded SQLite `chat` table, and exposes a basic health endpoint. Its embedded storage is only used for party chat history and one-year expiry.
