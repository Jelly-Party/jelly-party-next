...
test.describe("Video Swap Handling (Ad Simulation)", () => {
  ...
  test("should maintain sync when video source is swapped (ad insertion)", async ({ context }) => {
    ...
    // Get the magic link
    const magicLink = await chatIframeA.getByTestId("magic-link-input").inputValue();
    ...
    const preAdTimeA = await videoA.evaluate((v: HTMLVideoElement) => v.currentTime);
    const preAdDurationA = await videoA.evaluate((v: HTMLVideoElement) => v.duration);
    console.log(
    ...
    await pageA.waitForFunction(
      () => {
        const v = document.querySelector("video");
        return (
          v?.currentSrc?.includes("ad-video.mp4") && v.readyState >= 1 && !Number.isNaN(v.duration)
        );
      },
      { timeout: OPERATION_TIMEOUT },
    );
    ...
    const adDuration = await videoA.evaluate((v: HTMLVideoElement) => v.duration);
    ...
    const bTimeDuringAd = await videoB.evaluate((v: HTMLVideoElement) => v.currentTime);
    console.log(`B time during A's ad: ${bTimeDuringAd.toFixed(2)}s`);

    // B should still be on the main video, not affected by A's ad
    const bDuration = await videoB.evaluate((v: HTMLVideoElement) => v.duration);
    ...
    const postAdDurationA = await videoA.evaluate((v: HTMLVideoElement) => v.duration);
  ...
  test("should handle video element destruction and recreation", async ({ context }) => {
    // --- Setup: Create party on Tab A ---
    const pageA = await context.newPage();
    await pageA.goto(site.url, { waitUntil: "domcontentloaded" });
    ...
    await expect
      .poll(
        async () => {
          const aTime = await newVideoA.evaluate((v: HTMLVideoElement) => v.currentTime);
