...
test.describe("Party Sync Flow", () => {
  ...
  test(`Create and join party on ${defaultSite.displayName}`, async ({ context }) => {
    ...
    const magicLink = await chatIframe.getByTestId("magic-link-input").inputValue();
    ...
    await pageA.bringToFront();
    await expect(chatIframe.getByTestId("system-message").first()).toContainText(
      "joined the party",
      { timeout: OPERATION_TIMEOUT },
    );
    ...
    await expect(chatIframe.getByTestId("messages-container")).toContainText(testMessage);

    // Verify message appears in Tab B
    await pageB.bringToFront();
    await expect(chatIframeB.getByTestId("messages-container")).toContainText(testMessage, {
      timeout: OPERATION_TIMEOUT,
    });
    ...
    await expect(chatIframeB.getByTestId("messages-container")).toContainText(replyMessage);

    // Verify reply in Tab A
    await pageA.bringToFront();
    await expect(chatIframe.getByTestId("messages-container")).toContainText(replyMessage, {
      timeout: OPERATION_TIMEOUT,
    });
    ...
    await expect
      .poll(
        async () => {
          const bTime = await videoB.evaluate((v: HTMLVideoElement) => v.currentTime);
    ...
    await pageB.bringToFront();
    await expect
      .poll(async () => await videoB.evaluate((v: HTMLVideoElement) => v.paused), {
        timeout: OPERATION_TIMEOUT,
      })
      .toBe(false);
    ...
    await expect
      .poll(
        async () => {
          const aTime = await videoA.evaluate((v: HTMLVideoElement) => v.currentTime);
    ...
    await pageA.bringToFront();
    await expect
      .poll(async () => await videoA.evaluate((v: HTMLVideoElement) => v.paused), {
        timeout: OPERATION_TIMEOUT,
      })
      .toBe(false);
    ...
    await pageA.bringToFront();
    await expect(chatIframe.getByTestId("messages-container")).toContainText("left the party", {
      timeout: OPERATION_TIMEOUT,
    });
