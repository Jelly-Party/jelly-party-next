...
- Production uses the endpoints resolved from `config/urls.ts` (`https://www.jelly-party.com` and `wss://www.jelly-party.com` by default). Do not implement compatibility with the old extension or backend.
