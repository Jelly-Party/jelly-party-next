...
function findLargestVideo(): HTMLVideoElement | null {
  ...
  const bestVideo = withArea.find(({ area }) => area > 0)?.video ?? withArea[0]?.video ?? null;
...
export class VideoController {
  ...
  private onVideoChangeCallback: ((video: HTMLVideoElement | null) => void) | null = null;
  ...
  async play(timeFromEnd: number): Promise<boolean> {
    ...
    await DeferredPromise.withTimeout(this.deferredPlay, CONFIG.COMMAND_TIMEOUT_MS);
    return true;
  }
  ...
  async pause(timeFromEnd: number): Promise<boolean> {
    ...
    await DeferredPromise.withTimeout(this.deferredPause, CONFIG.COMMAND_TIMEOUT_MS);
    return true;
  }
  ...
  async seek(timeFromEnd: number): Promise<boolean> {
    ...
    await DeferredPromise.withTimeout(this.deferredSeek, CONFIG.COMMAND_TIMEOUT_MS);
  ...
  private isVideoValid(): boolean {
    return Boolean(
      this.video?.isConnected && (this.video.offsetWidth > 0 || this.video.offsetHeight > 0),
    );
  }
