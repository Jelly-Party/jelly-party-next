...
export async function loadSyncOptions(): Promise<SyncOptionsConfig> {
  try {
    ...
    const stored = result[STORAGE_KEY] as Partial<SyncOptionsConfig> | undefined;
 ...
 */
export async function saveSyncOptions(options: Partial<SyncOptionsConfig>): Promise<void> {
  try {
