...
const WS_URL = import.meta.env.VITE_JELLY_LOG_URL || "ws://localhost:3000";
...
function parseStructuredLog(
  // oxlint-disable-next-line typescript/no-explicit-any -- Console arguments are unconstrained.
  args: any[],
  ...
  try {
    ...
    if (parsed.level && parsed.msg) {
      const { level, msg, ts: _ts, ...data } = parsed;
      return {
        level,
        msg,
...
export function initDevLogger() {
  ...
  const processConsoleCall = (
    // oxlint-disable-next-line typescript/no-explicit-any -- Mirrors the Console API.
  ...
  console.debug = (...args) => processConsoleCall(originalDebug, "debug", ...args);
  console.info = (...args) => processConsoleCall(originalInfo, "info", ...args);
  console.warn = (...args) => processConsoleCall(originalWarn, "warn", ...args);
  console.error = (...args) => processConsoleCall(originalError, "error", ...args);
}
