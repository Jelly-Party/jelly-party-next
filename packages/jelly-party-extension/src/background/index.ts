...
const adjectives = ["Happy", "Silly", "Brave", "Clever", "Swift", "Gentle", "Wild", "Calm"];
const animals = ["Panda", "Koala", "Fox", "Owl", "Tiger", "Dolphin", "Eagle", "Wolf"];
 ...
 */
async function redirectToParty(tabId: number, redirectURL: string, partyId: string): Promise<void> {
  ...
  const navigationComplete = new Promise<void>((resolve) => {
    const listener = (tid: number, changeInfo: browser.Tabs.OnUpdatedChangeInfoType) => {
...
browser.runtime.onMessage.addListener((message: unknown, sender: browser.Runtime.MessageSender) => {
  ...
  switch (msg.type) {
    ...
    case "getPartyId":
      return browser.storage.local.get("currentPartyId").then((r) => r.currentPartyId);
    ...
    case "redirectToParty": {
      ...
      log.info("Received redirectToParty", payload);
      const tabId = sender.tab?.id;
    ...
    case "checkPermission": {
      ...
      return browser.permissions.contains({ origins: [origin] }).then((hasPermission) => {
